...
class SCMExploratoryAnalysis:
    ...
    def basic_data_overview(self):
        ...
        revenue_col = 'cost_of_goods_sold_cogs'
        if revenue_col in self.df.columns:
            revenue_values = self.df[revenue_col].value_counts().head(10)
            for value, count in revenue_values.items():
    ...
    def analyze_supplier_metrics(self):
        """Analyze supplier-related metrics"""
        print("\n" + "="*60)
        print("🤝 SUPPLIER ANALYSIS")
        print("="*60)
        
        supplier_cols = ['supplier_count', 'supplier_collaboration_level', 'supplier_relationship_score']
        
        for col in supplier_cols:
            if col in self.df.columns:
                if col == 'supplier_count':
                    # Handle mixed data types (some might be strings with commas)
                    try:
                        # Convert to numeric, handling comma-separated values
                        supplier_counts = pd.to_numeric(self.df[col].astype(str).str.replace(',', ''), errors='coerce')
                        valid_counts = supplier_counts.dropna()
                        
                        if len(valid_counts) > 0:
                            print(f"\n📊 {col.replace('_', ' ').title()}:")
                            print(f"  Mean: {valid_counts.mean():.0f}")
                            print(f"  Median: {valid_counts.median():.0f}")
                            print(f"  Max: {valid_counts.max():.0f}")
                            print(f"  Companies with valid data: {len(valid_counts)}")
                            
                            # Create box plot
                            plt.figure(figsize=(10, 6))
                            plt.boxplot(valid_counts, patch_artist=True, boxprops=dict(facecolor='lightblue'))
                            plt.title(f'Distribution of {col.replace("_", " ").title()}', fontsize=14, fontweight='bold')
                        ...
                        print(f"⚠️ Could not analyze {col} due to data format issues")
                
                elif col in ['supplier_collaboration_level', 'supplier_relationship_score']:
                    if self.df[col].dtype == 'object':
                        # Categorical analysis
                        levels = self.df[col].value_counts()
                        print(f"\n📊 {col.replace('_', ' ').title()}:")
                        ...
                        # Create bar plot
                        plt.figure(figsize=(10, 6))
                        levels.plot(kind='bar', color='lightcoral', edgecolor='black')
                        plt.title(f'Distribution of {col.replace("_", " ").title()}', fontsize=14, fontweight='bold')
    ...
    def analyze_operational_efficiency(self):
        ...
        efficiency_cols = ['operational_efficiency_score', 'inventory_turnover_ratio', 'order_fulfillment_rate_']
        ...
        for col in efficiency_cols:
            if col in self.df.columns:
                ...
                # Create histogram
                plt.figure(figsize=(10, 6))
                plt.hist(self.df[col], bins=20, color='gold', edgecolor='black', alpha=0.7)
                plt.title(f'Distribution of {col.replace("_", " ").title()}', fontsize=14, fontweight='bold')
                plt.xlabel('Score/Ratio', fontsize=12)
                plt.ylabel('Number of Companies', fontsize=12)
                plt.axvline(self.df[col].mean(), color='red', linestyle='--', label=f'Mean: {self.df[col].mean():.2f}')
                plt.legend()
                plt.tight_layout()
    ...
    def top_performers_analysis(self):
        ...
        if 'inventory_turnover_ratio' in self.df.columns:
            top_inv = self.df.nlargest(10, 'inventory_turnover_ratio')[['company_name', 'inventory_turnover_ratio']]
            print("\n📦 Top 10 Inventory Turnover:")
            for idx, row in top_inv.iterrows():
                print(f"  {row['company_name']}: {row['inventory_turnover_ratio']:.1f}")
    ...
    def generate_eda_summary(self):
        ...
        summary.append("  - Supplier metrics analysis")
        summary.append("  - Operational efficiency analysis")
